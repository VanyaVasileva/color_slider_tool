...
st.caption("Upload a transparent PNG, slide across the color bar, and export directly.")
...
<style>
  ...
  * { box-sizing: border-box; }
  ...
  /* Fixed, square preview frame. Width is responsive (max 560px), but
     position is always centered (margin:0 auto) and the size never
     changes during color picking - only on browser resize. */
  .preview-box {
    position: relative;
    width: 100%;
    max-width: 560px;
...
<div class="app">
  <div class="card upload">
    <input id="fileInput" type="file" accept="image/png" aria-label="Upload transparent PNG" />
    <div class="hint">The PNG should have a transparent background.</div>
  </div>
  ...
  <div class="card">
    ...
    <div id="status">No PNG loaded yet.</div>
  </div>
  ...
  <div class="card">
    ...
    <button class="primary" id="downloadBtn" type="button" disabled>Save PNG with background</button>
    <div class="hint">The export keeps the original size of your uploaded PNG.</div>
  </div>
</div>
...
(() => {
  ...
  // Sets the canvas resolution (pixel buffer) to match the fixed frame size.
  // Called ONLY on first load and on an actual window resize - never on
  // image upload or color picking. This is what keeps the preview from
  // ever shifting while sliding across the palette.
  ...
  // Paints pixels only (background color + image "contain"-scaled and
  // centered in the fixed frame). Never touches the canvas size.
  ...
  fileInput.addEventListener('change', () => {
    ...
    img.onload = () => {
      ...
      status.textContent = `${img.naturalWidth} × ${img.naturalHeight} px loaded`;
      URL.revokeObjectURL(url);
    };
    img.onerror = () => {
      status.textContent = 'The PNG could not be loaded.';
      URL.revokeObjectURL(url);
    };
    img.src = url;
  });
